    ...
    <div class="card shadow-sm">
      <div class="card-body">
        <h1 class="card-title mb-4">
          ...
          <img :src="imgUrl" class="prdtimg" alt="" />
        </h1>
        <p v-if="isSub" class="text-success">
          <strong>이 상품에 이미 가입하셨습니다.</strong>
        </p>
        <p class="card-text">
          <strong class="text-primary">금융회사명:</strong>
          {{ productDetail.product?.kor_co_nm }}
        </p>
        <p class="card-text">
          <strong class="text-primary">가입대상:</strong>
          {{ productDetail.product?.join_member }}
        </p>
        <p class="card-text"><strong class="text-primary">우대조건:</strong></p>
        <p
          v-for="phase in productDetail.product?.spcl_cnd.split('\n')"
          :key="phase"
          class="card-text"
        >
          {{ phase }}
        ...
        <p class="card-text">
          <strong class="text-primary">만기 후 이율:</strong>
        </p>
        <p
          v-for="phase in productDetail.product?.mtrt_int.split('\n')"
          :key="phase"
          class="card-text"
        >
          {{ phase }}
        ...
        <p class="card-text">
          <strong class="text-primary">최고한도:</strong>
          {{
            productDetail.product?.max_limit
              ? (productDetail.product?.max_limit)
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원"
              : "-"
        ...
        <p class="card-text">
          <strong class="text-primary">가입방식:</strong>
          {{ productDetail.product?.join_way }}
        </p>
        <p class="card-text">
          <strong class="text-primary">기타 사항:</strong>
        </p>
        <p
          v-for="phase in productDetail.product?.etc_note.split('\n')"
          :key="phase"
          class="card-text"
        >
          {{ phase }}
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" @click="confirmInput">
              확인
            </button>
          </div>
        </div>
  ...
  if (productDetail.value.product?.fin_prdt_nm.includes("예금")) {
    ...
    if (selectedOption.value.intr_rate_type_nm.includes("단리")) {
      ...
      expectedAmount.value = (
        principal +
        principal * interestRate * periodYears
      ).toFixed(2);
    } else {
      // 복리 계산
      expectedAmount.value = (
        principal * Math.pow(1 + interestRate / 12, periodMonths)
      ).toFixed(2);
    }
  } else {
    // 적금 방식 계산
    ...
    if (selectedOption.value.intr_rate_type_nm.includes("단리")) {
      // 단리 계산
      expectedAmount.value = (
        monthlyDeposit * periodMonths +
        (monthlyDeposit *
          interestRate *
          ((periodMonths * (periodMonths + 1)) / 2)) /
          12
      ).toFixed(2);
    } else {
      // 복리 계산
      ...
      for (let i = 1; i <= periodMonths; i++) {
        totalAmount +=
          monthlyDeposit *
          Math.pow(1 + interestRate / 12, periodMonths - i + 1);
      }
...
const subscribe = () => {
  if (DorS.value === "deposit") {
    axios({
      ...
      url: `http://${
        import.meta.env.VITE_ENV_BACK_END_URL
      }:8000/products/subscribed-${DorS.value}/`,
      data: {
