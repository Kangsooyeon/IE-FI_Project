<template>
  <div class="container my-4 d-flex flex-column align-items-center">
    <div class="article-detail card mb-4 p-4 shadow-sm">
      ...
      <p class="text-muted">
        작성일:
        {{ new Date(store.articleDetail.created_at).toLocaleDateString() }}
      </p>
      <div class="d-flex flex">
        <button
          v-if="
            store.articleDetail.nickname === store.userInfo.nickname &&
            !editArticleMode
          "
          @click="editArticle"
          class="btn update btn-secondary mt-3"
        >
          수정하기
        </button>
        <button
          ...
          class="btn delete btn-danger mt-3"
        >
          삭제하기
        </button>
        <button
          v-if="editArticleMode"
          @click="saveArticle"
    ...
    <div class="comments-section card p-4 shadow-sm">
      ...
      <div
        ...
        :key="comment.id"
        class="comment mb-3 p-2 border-bottom d-flex justify-content-between"
      >
        <div>
          <p v-if="!editMode[comment.id]" class="mb-1 comment-content">
            {{ comment.content }}
          </p>
        ...
        <div
          v-if="comment.nickname === store.userInfo.nickname"
          class="d-flex justify-content-end"
        >
          <button
            v-if="!editMode[comment.id]"
            @click="editComment(comment)"
            class="btn btn-sm btn-secondary comment-btn mr-2"
          >
            수정
          </button>
          <button
            ...
            class="btn btn-sm btn-primary comment-btn mr-2"
          >
            저장
          </button>
          <button
            @click="confirmDeleteComment(comment.id)"
            class="btn btn-sm btn-danger comment-btn"
          >
            삭제
          </button>
        </div>
...
const addComment = () => {
  if (store.isLogin === false) {
    ...
    axios({
      ...
      }:8000/articles/${article_id}/comment-create/`,
      data: {
...
const saveComment = (comment) => {
  ...
  axios({
    ...
    url: `http://${
      import.meta.env.VITE_ENV_BACK_END_URL
    }:8000/articles/${article_id}/comment-update/${comment.id}/`,
    data: {
...
const deleteComment = (commentId) => {
  axios({
    ...
    url: `http://${
      import.meta.env.VITE_ENV_BACK_END_URL
    }:8000/articles/${article_id}/comment-delete/${commentId}/`,
    headers: {
      Authorization: `Token ${store.token}`,
    },
  }).then((res) => {
...
const saveArticle = () => {
  ...
  axios({
    ...
    url: `http://${
      import.meta.env.VITE_ENV_BACK_END_URL
    }:8000/articles/${article_id}/update/`,
    data: {
...
const goDelete = () => {
  axios({
    ...
    url: `http://${
      import.meta.env.VITE_ENV_BACK_END_URL
    }:8000/articles/${article_id}/delete/`,
    headers: {
      Authorization: `Token ${store.token}`,
    },
  })
